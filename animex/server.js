...
_shows=[];_sections=[];_map={};_loaded=true;
}
_loading=false;
_cbs.forEach(r=>r());
_cbs=[];
}
function onReady(cb){if(_loaded)cb();else{_load().then(cb);}}
return{
langLabels:LL,
...
getContinueWatching(){const out=[],p=gPR();Object.keys(p).forEach(sid=>{const s=_map[sid];if(!s)return;Object.keys(p[sid]).forEach(eid=>{const pct=p[sid][eid];if(pct>2&&pct<95){const ep=s.episodes.find(x=>x.id===eid);if(ep)out.push({show:s,ep,pct});}});});return out;}
};
})();