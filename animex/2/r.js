...
const _mdo=()=>document.getElementById('mdo');
const _mro=()=>document.getElementById('mro');
let _stack=[];
...
if(pg==='show'){const evoEl=_evo();if(evoEl&&evoEl.classList.contains('op')){NV.destroy();evoEl.classList.remove('op');_stack=_stack.filter(z=>z!==700);document.body.style.overflow='';setTimeout(()=>{evoEl.innerHTML='';show(parts[1],false);},300);}else{show(parts[1],false);}return;}
if(pg==='ep'){ep(parts[1],parts[2],false);return;}
if(pg==='see'){seeAll(decodeURIComponent(parts[1]),false);return;}
if(pg==='manga'){manga(parts[1],false);return;}
if(pg==='chapter'){chapter(parts[1],parts[2],false);return;}
...
function _closeAll(){[_sdo(),_evo(),_sao(),_mdo()].forEach(el=>{if(el&&el.classList.contains('op')){el.classList.remove('op');setTimeout(()=>{el.innerHTML='';},380);}});if(_evo()&&_evo().classList.contains('op'))NV.destroy();if(_mro()&&_mro().classList.contains('op'))MR.destroy();_stack=[];document.body.style.overflow='';}
...
/* ── MANGA DETAIL ─────────────────────────────────────────────────── */
function manga(id,push=true){
D.onReady(()=>{
const m=D.getManga(id);if(!m)return;
if(push)history.pushState(null,'','#manga/'+id);
const el=_mdo();if(!el)return;
const wl=D.wishlist.has(id);
const chs=m.chapters||[];
const statusHtml=m.status?`<span class="mg-status ${m.status.toLowerCase()}">${m.status}</span>`:'';
let h=`<div class="sdoh"><img class="sdohi li" src="${D.heroImg(m)}" alt="${m.title}"><div class="sdoho"></div><div class="sdocl" onclick="R.closeManga()">${IC.close}</div></div>
<div class="sdob">
<div class="sdolog">${m.title}</div>
<div class="sdomt">${m.year} · ${chs.length} Ch.${statusHtml}${m.author?' · '+m.author:''} · ⭐ ${m.rating}</div>
${chs.length?`<button class="sdopl" onclick="R.chapter('${id}','${chs[0].id}')">${IC.play} Read Now · Ch.${chs[0].ch}</button>`:''}
<div class="sdogns">${(m.genres||[]).map(g=>`<span class="sdogn">${g}</span>`).join('')}</div>
<div class="sdodsc">${m.desc||''}</div>
<div class="sdoacts">
<button class="sdoact wlb${wl?' on':''}" data-wl-s="${id}" onclick="P._twls('${id}',this)">${(wl?IC.check:IC.bookmark)}<span>Watchlist</span></button>
<button class="sdoact" onclick="P.share('${id}')">${IC.share}<span>Share</span></button>
<button class="sdoact" onclick="P.openRating('${id}')">${IC.heart}<span>Rate</span></button>
</div>
<div class="sch">Chapters</div>
${P.buildMangaChapters(id,m)}
<div class="sch">More Manga</div><div class="mlts">`;
D.mangas.filter(x=>x.id!==id).slice(0,6).forEach(mx=>{h+=P._mangaCard(mx);});
h+=`</div></div>`;
el.innerHTML=h;
const z=601;el.style.zIndex=z;_stack.push(z);
document.body.style.overflow='hidden';
requestAnimationFrame(()=>el.classList.add('op'));
P.obsL(el);P.setMeta(id);
});}

function closeManga(){
const el=_mdo();if(!el)return;
el.classList.remove('op');_stack=_stack.filter(z=>z!==601);
document.body.style.overflow='';
setTimeout(()=>{el.innerHTML='';},380);
const h=location.hash.slice(1);if(h.startsWith('manga/'))history.back();
}

/* ── MANGA CHAPTER READER ─────────────────────────────────────────── */
function chapter(mid,chid,push=true){
D.onReady(()=>{
const m=D.getManga(mid);if(!m)return;
const ch=D.getMangaChapter(mid,chid);if(!ch)return;
if(push)history.pushState(null,'','#chapter/'+mid+'/'+chid);
const mro=_mro();if(!mro)return;
_stack.push(800);
document.body.style.overflow='hidden';
MR.init(mid,chid,m,ch);
P.setMeta(mid);
});}

function closeChapter(){
MR.destroy();
_stack=_stack.filter(z=>z!==800);
document.body.style.overflow='';
const h=location.hash.slice(1);if(h.startsWith('chapter/'))history.back();
}

function ep(sid,eid,push=true){D.onReady(()=>{
const s=D.getShow(sid);if(!s)return;
const epData=D.getEp(sid,eid);if(!epData)return;
const evoEl=_evo(),isOp=evoEl.classList.contains('op');
if(isOp){
const wasFs=!!document.fullscreenElement;
_cShow=sid;_cEp=eid;
NV.destroy(true);
_bldEvo(s,epData,sid,evoEl);
NV.init(sid,eid,s,epData);
if(wasFs)setTimeout(()=>{const nr=document.getElementById('nr');if(nr&&!document.fullscreenElement)nr.requestFullscreen().catch(()=>{});},150);
return;
}
if(push)history.pushState(null,'','#ep/'+sid+'/'+eid);
_cShow=sid;_cEp=eid;
_bldEvo(s,epData,sid,evoEl);
const z=700;evoEl.style.zIndex=z;_stack.push(z);
document.body.style.overflow='hidden';
requestAnimationFrame(()=>evoEl.classList.add('op'));
NV.init(sid,eid,s,epData);
P.setMeta(sid,eid);
});}
...
const fsLeft=mv
?`<button class="nbfs-btn" id="nwn">${IC.watchNext}<span>Watch Next</span></button>`
:`<button class="nbfs-btn" id="nwn">${IC.watchNext}<span>Watch Next</span></button><button class="nbfs-btn" id="nep">${IC.episodes}<span>Episodes</span></button>`;
const fsRight=mv
?`<button class="nbfs-btn" id="nspd">Speed&nbsp;<span id="nspd-lbl">1x</span></button>`
:`<button class="nbfs-btn" id="nspd">Speed&nbsp;<span id="nspd-lbl">1x</span></button><button class="nbfs-btn" id="nnx">${IC.skipNext}<span>Next Ep</span></button>`;
const videoHtml=`<div id="np-page"><div id="nr" role="region" tabindex="-1">
<video id="nvid" preload="metadata" playsinline webkit-playsinline controlslist="nodownload noremoteplayback" disablepictureinpicture oncontextmenu="return false"></video>
<img id="nth" alt="" draggable="false">
<div id="nsp" role="status"></div>
<div id="nsh2"></div>
<div id="nnl"></div><div id="nnr"></div>
<div id="ng-left"></div>
<div id="ng-right"></div>
<div id="ng-toast"></div>
<div id="nsrc-badge"></div>
<div id="nov" role="presentation" aria-hidden="true"></div>
<div id="nct">
  <div id="nct-left">
    <button class="nb nct-back" onclick="R.closeEp()" title="Back">${IC.chevDown}</button>
    <div id="nct-title"><span id="nct-title-main"></span><span id="nct-title-sub"></span></div>
  </div>
  <div id="nct-right">
    ...
    <button class="nb nct-nofs-only" id="nfs-btn" title="Fullscreen">${IC.fullscreen}</button>
  </div>
</div>
<div id="ncc">
...
const ymlCard=ms=>`<div class="sc" onclick="R.closeEp();setTimeout(()=>R.show('${ms.id}'),350)"><div class="sc-i"><div class="sc-img-w"><img src="${ms.thumb||''}" class="sci li" alt="${ms.title}"></div><div class="sc-gl"></div><div class="sc-bot"><div class="sc-t">${ms.title}</div><div class="sc-m">⭐${ms.rating} · ${ms.year}</div></div></div></div>`;
...
evoEl.innerHTML=h;
requestAnimationFrame(()=>{evoEl.querySelectorAll('img[data-src]').forEach(img=>{img.src=img.dataset.src||'';img.classList.add('li');delete img.dataset.src;});});
P.obsL(evoEl);if(isDesk&&!mv)_bindDeskPl(s,sid,epData);
}
...
function _bldDeskEpList(s,season,curId,sid){return s.episodes.filter(e=>e.s===season).map(e=>{const pct=D.getProgress(sid,e.id),act=e.id===curId;return`<div class="evo-pl-ep${act?' act':''}" onclick="R.ep('${sid}','${e.id}')"><div class="evo-pl-th"><img src="${e.thumb||''}" alt="${e.title}" class="li"><div class="evo-pl-pb"><div class="evo-pl-pbf" style="width:${pct}%"></div></div></div><div class="evo-pl-meta"><div class="evo-pl-en">${e.title}</div><div class="evo-pl-em">E${e.e} · ${e.dur}</div></div></div>`;}).join('');}
...
function _deskPlSeason(sid,season,te){document.querySelectorAll('#evo-pl-panel-stabs .stab').forEach(t=>t.classList.remove('act'));te.classList.add('act');const s=D.getShow(sid);const list=document.getElementById('evo-pl-panel-list');if(list&&s){list.innerHTML=_bldDeskEpList(s,season,_cEp,sid);requestAnimationFrame(()=>{list.querySelectorAll('img[data-src]').forEach(i=>{i.src=i.dataset.src||'';i.classList.add('li');delete i.dataset.src;});});P.obsL(list);}}
function _evoss(sid,n,te){document.querySelectorAll('#evostabs .stab').forEach(t=>t.classList.remove('act'));te.classList.add('act');const c=document.getElementById('evoepsl');c.innerHTML=P.buildEps(sid,n,_cEp);requestAnimationFrame(()=>{c.querySelectorAll('img[data-src]').forEach(i=>{i.src=i.dataset.src||'';i.classList.add('li');delete i.dataset.src;});});P.obsL(c);}
...
return{init,show,ep,manga,chapter,seeAll,closeShow,closeEp,closeManga,closeChapter,closeSeeAll,sSeason,_evoss,_deskPlSeason};
})();