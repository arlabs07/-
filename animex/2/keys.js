...
function _mdoOpen(){const e=document.getElementById('mdo');return e&&e.classList.contains('op');}
function _mroOpen(){const e=document.getElementById('mro');return e&&e.classList.contains('op');}
...
// Close overlays with Escape — priority: deepest overlay first
if(k==='Escape'){
if(_evoOpen()){R.closeEp();return;}
if(_mroOpen()){R.closeChapter();return;}
if(_plFsOpen()){P.closePlFull();return;}
if(_sdoOpen()){R.closeShow();return;}
if(_mdoOpen()){R.closeManga();return;}
...
// Manga reader shortcuts — only when manga reader is open
// (MR itself also binds _kh, these are additional shortcuts via keys.js)
if(_mroOpen()){
switch(c){
case'ArrowRight':case'ArrowDown':e.preventDefault();MR._next();break;
case'ArrowLeft':case'ArrowUp':e.preventDefault();MR._prev();break;
case'KeyF':e.preventDefault();MR._toggleFS();break;
case'KeyT':e.preventDefault();MR._toggleThumbs();break;
case'KeyW':e.preventDefault();MR._toggleMode();break;
}
return;
}
// Global page navigation — only when no overlay open
if(!_sdoOpen()&&!_saoOpen()&&!_plFsOpen()&&!_mdoOpen()&&!_mroOpen()){
switch(k){
...
case'?':P.toast('Shortcuts: 1/2/3=Pages  /=Search  Esc=Back  Space/K=Play  ←→=Seek  F=Full  M=Mute  ,/.=Speed  W=Webtoon mode  T=Thumbnails');break;
}
}
});
}
