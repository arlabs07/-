/* AnimeX · r.js — Router v4
   - Overlay backdrop blur (body::after pseudo)
   - Correct z-index stacking
   - No download icons in episode rows
...
const R = (() => {
  ...
  /* Body backdrop when overlays open */
  ...
  function _closeAll(){
    ...
    if(sdo.classList.contains('open')){sdo.classList.remove('open');setTimeout(()=>{sdo.innerHTML='';},380);}
    if(evo.classList.contains('open')){evo.classList.remove('open');NV.destroy();setTimeout(()=>{evo.innerHTML='';},380);}
    if(sao.classList.contains('open')){sao.classList.remove('open');setTimeout(()=>{sao.innerHTML='';},380);}
  ...
  function _route(h){
    ...
    const page=document.getElementById('page-'+pg);if(!page)return;
  ...
  function show(id,push=true){
    ...
    <div class="sdoh">
      ...
      <div class="sdoho"></div>
      <div class="sdoclose" onclick="R.closeShow()"><span class="material-icons">close</span></div>
    </div>
    <div class="sdob">
      <div class="sdologo">${s.title||'Untitled'}</div>
      ...
      <button class="sdoplay" onclick="R.ep('${id}','${s.episodes[0].id}')">
        <span class="material-icons">play_arrow</span> Watch · S${s.episodes[0].s} E${s.episodes[0].e}
      </button>
      <div class="sdogens">${s.genres.map(g=>`<span class="sdogen">${g}</span>`).join('')}</div>
      <div class="sdodesc">${s.desc}</div>
      <div class="sdoacts">
        <button class="sdoact wl-btn${wl?' on':''}" data-wl-show="${id}" onclick="P._toggleWLShow('${id}',this)">
          <span class="material-icons">${wl?'check':'bookmark_border'}</span><span>Watchlist</span>
        </button>
        ...
        <button class="sdoact" onclick="P.toast('Rated!')">
          <span class="material-icons">favorite_border</span><span>Rate</span>
        </button>
      </div>
    ...
    h+=`</div><div id="eps-list">${P.buildEps(id,1)}</div>
  ...
  function closeShow(){
    const el=_sdo();el.classList.remove('open');
    document.body.style.overflow='';_setBackdrop(false);
  ...
  function ep(showId,epId,push=true){
    ...
    const evoEl=_evo(),isOpen=evoEl.classList.contains('open');
    if(isOpen){_curShow=showId;_curEp=epId;NV.destroy();_buildEvoBody(s,epData,showId,evoEl);NV.init(showId,epId,s,epData);return;}
  ...
  function _buildEvoBody(s,epData,showId,evoEl){
    ...
    let h=`
    <div id="evo-topbar">
      <div id="evo-topbar-back" onclick="R.closeEp()"><span class="material-icons">expand_more</span></div>
      <div id="evo-topbar-title">${s.title||'Untitled'} · S${epData.s} E${epData.e}</div>
    </div>
    <div id="np-page">
    ...
    <div id="evo-body">
      <div class="evot">${epData.title||'Untitled'}</div>
      ...
      <div class="evoacts">
        <button class="evoact wl-btn${wl?' on':''}" data-wl-show="${showId}" onclick="P._toggleWLShow('${showId}',this)">
          <span class="material-icons">${wl?'check':'bookmark_border'}</span><span>Watchlist</span>
        </button>
      ...
      <div class="sec-heading">${s.title||'Untitled'} — Episodes</div>
      <div class="stabs" id="evo-stabs">`;
    for(let i=1;i<=s.seasons;i++)h+=`<div class="stab${i===epData.s?' act':''}" onclick="R._evoSwitchSeason('${showId}',${i},this)">Season ${i}</div>`;
    h+=`</div><div id="evo-eps-list">${P.buildEps(showId,epData.s,epData.id)}</div>`;
    const related=D.shows.filter(x=>x.id!==showId).slice(0,8);
    if(related.length){h+=`<div class="sec-heading">You May Like</div><div class="secr" style="padding:0 0 8px">`;related.forEach(ms=>h+=P._card(ms));h+=`</div>`;}
    h+=`</div>`;
    evoEl.innerHTML=h;P.observeLazy(evoEl);
  }
  ...
  function _evoSwitchSeason(showId,n,tabEl){
    ...
    container.innerHTML=P.buildEps(showId,n,_curEp);P.observeLazy(container);
  }

  function closeEp(){
    NV.destroy();const el=_evo();el.classList.remove('open');
    document.body.style.overflow='';_setBackdrop(false);
    setTimeout(()=>{el.innerHTML='';},380);
    const hash=location.hash.slice(1);if(hash.startsWith('ep/'))history.back();
  }
  ...
  function seeAll(title,push=true){
    ...
    const el=_sao();
    let h=`<div class="saotb">
      <div class="saotb-back" onclick="R.closeSeeAll()"><span class="material-icons">chevron_left</span></div>
      <div class="saotb-title">${title}</div>
    </div>
    <div class="saobody"><div class="saogrid">`;
  ...
  function closeSeeAll(){
    const el=_sao();el.classList.remove('open');
    document.body.style.overflow='';_setBackdrop(false);
    setTimeout(()=>{el.innerHTML='';history.back();},380);
  }
