...
function nudge(txt,side){const t=side==='l'?el.nul:side==='r'?el.nur:el.nu;t.textContent=txt;t.classList.remove('snp');t.offsetWidth;t.classList.add('snp');clearTimeout(_nt);_nt=setTimeout(()=>{el.nu.classList.remove('snp');el.nul.classList.remove('snp');el.nur.classList.remove('snp');},680);}
function showUI(){if(!ST.started)return;ST.uiFull=true;el.bot.classList.add('nv');el.ct.classList.add('nv');if(el.vid.paused&&!ST.ended)el.cp.classList.add('ns');hiTmr();}
function hideUI(){if(!ST.started)return;ST.uiFull=false;el.bot.classList.remove('nv');el.ct.classList.remove('nv');el.cp.classList.remove('ns');cMenus();}
...
function sMn(k,v){const M={s:[el.spm,'spdOpen'],l:[el.lm,'langOpen'],q:[el.qm,'qualOpen'],p:[el.plp,'plOpen']};const on=v!==undefined?v:!ST[M[k][1]];if(on)Object.keys(M).filter(x=>x!==k).forEach(x=>{M[x][0].classList.remove('no');ST[M[x][1]]=false;});M[k][0].classList.toggle('no',on);ST[M[k][1]]=on;if(on&&k==='s')setTimeout(updSpd,50);}
function tPlay(){if(ST.ended){el.vid.currentTime=0;el.vid.play();ST.ended=false;}else{el.vid.paused?el.vid.play():el.vid.pause();}}
function seek(d){el.vid.currentTime=Math.max(0,Math.min(el.vid.duration||0,el.vid.currentTime+d));nudge(d>0?`+${d}s`:`${d}s`,d>0?'r':'l');}
function setVol(v){ST.vol=Math.max(0,Math.min(1,v));el.vid.volume=ST.vol;ST.muted=ST.vol===0;el.vid.muted=ST.muted;updVol();}
function tMute(){ST.muted=!ST.muted;el.vid.muted=ST.muted;updVol();nudge(ST.muted?'Muted':'Unmuted','c');}
function setSpd(v){ST.spd=Math.max(.25,Math.min(3,v));el.vid.playbackRate=ST.spd;updSpd();nudge(`${ST.spd}x`,'c');}
function updVol(){const v=ST.muted?0:ST.vol;el.vf.style.width=100*v+'%';el.vs.value=v;el.vol.innerHTML=v===0?IC.mute:IC.volume;}
function updSpd(){document.querySelectorAll('.nmi[data-s]').forEach(b=>{const n=parseFloat(b.dataset.s)===ST.spd;b.classList.toggle('na',n);b.innerHTML=n?`${IC.check}<span>${b.dataset.s}x</span>`:`${b.dataset.s}x`;if(n&&ST.spdOpen)b.scrollIntoView({block:'nearest'});});el.spb.querySelector('span').textContent=ST.spd+'x';}
function sFrac(f){const d=el.vid.duration;if(isFinite(d))el.vid.currentTime=Math.max(0,Math.min(d,f*d));}
...
function tick(){const d=el.vid.duration||0,c=el.vid.currentTime||0,f=d?c/d:0;el.sf.style.width=100*f+'%';el.sth.style.left=100*f+'%';el.tm.textContent=`${fT(c)} / ${fT(d)}`;if(el.vid.buffered.length>0&&d>0){let b=0;for(let i=0;i<el.vid.buffered.length;i++)if(el.vid.buffered.start(i)<=c&&el.vid.buffered.end(i)>=c){b=el.vid.buffered.end(i);break;}el.sbuf.style.width=b/d*100+'%';}if(d>0)D.setProgress(_sid,_eid,Math.min(100,Math.round(f*100)));ST.raf=requestAnimationFrame(tick);}
function loadTrk(lk,qk,resume){const tr=_show.tracks,langs=Object.keys(tr);if(!langs.includes(lk))lk=langs[0];ST.lang=lk;const quals=Object.keys(tr[lk]);if(!quals.includes(qk))qk=quals[0];ST.qual=qk;const src=tr[lk][qk],ct=resume?el.vid.currentTime:0,wp=!el.vid.paused;el.vid.src=src;el.vid.load();el.vid.currentTime=ct;ST.ended=false;if(wp&&resume)el.vid.play().catch(()=>{});el.th.src=_ep.thumb;el.th.style.display='block';if(el.cth)el.cth.src=_ep.thumb;el.fst.textContent=`${_show.title} · ${_ep.title}`;bldLng(langs);bldQual(quals);el.lb.querySelector('span').textContent=lk.toUpperCase();el.qb.querySelector('span').textContent=qk;el.lb.style.display=langs.length>1?'flex':'none';el.qb.style.display=quals.length>1?'flex':'none';}
function bldLng(ls2){el.lm.innerHTML='';ls2.forEach(lk=>{const b=document.createElement('button');b.className='nmi';b.dataset.l=lk;const label=D.langLabels[lk]||lk.toUpperCase(),act=lk===ST.lang;b.classList.toggle('na',act);b.innerHTML=act?`${IC.check}<span>${label}</span>`:label;b.onclick=e=>{e.stopPropagation();if(ST.lang!==lk){loadTrk(lk,ST.qual,true);nudge(label,'c');}sMn('l',false);};el.lm.appendChild(b);});}
function bldQual(qs){el.qm.innerHTML='';qs.forEach(qk=>{const b=document.createElement('button');b.className='nmi';b.dataset.q=qk;const act=qk===ST.qual;b.classList.toggle('na',act);b.innerHTML=act?`${IC.check}<span>${qk}</span>`:qk;b.onclick=e=>{e.stopPropagation();if(ST.qual!==qk){loadTrk(ST.lang,qk,true);nudge(qk,'c');}sMn('q',false);};el.qm.appendChild(b);});}
function bldSpd(){el.spm.innerHTML='';CFG.speeds.forEach(v=>{const b=document.createElement('button');b.className='nmi';b.dataset.s=v;b.textContent=v+'x';b.onclick=e=>{e.stopPropagation();setSpd(v);sMn('s',false);};el.spm.appendChild(b);});}
function bldPl(){el.pll.innerHTML='';_pl.forEach(ep=>{const li=document.createElement('li');li.className='nppi';li.innerHTML=`<div class="npit"><img src="${ep.thumb||''}"></div><div class="npim"><span class="npin">S${ep.s} E${ep.e} · ${ep.title}</span><span class="npis">${ep.dur||''}</span></div>`;li.classList.toggle('na',ep.id===_eid);li.onclick=()=>{sMn('p',false);R.ep(_sid,ep.id);};el.pll.appendChild(li);});}
let _kb=false;
function bindKB(){if(_kb)return;_kb=true;document.addEventListener('keydown',e=>{if(e.target.tagName==='INPUT'||e.target.tagName==='TEXTAREA')return;const ev=document.getElementById('evo');if(!ev||!ev.classList.contains('op'))return;switch(e.code){case'Space':case'KeyK':e.preventDefault();tPlay();showUI();break;case'ArrowRight':e.preventDefault();seek(CFG.skip);showUI();break;case'ArrowLeft':e.preventDefault();seek(-CFG.skip);showUI();break;case'ArrowUp':e.preventDefault();setVol(Math.min(1,ST.vol+.1));showUI();break;case'ArrowDown':e.preventDefault();setVol(Math.max(0,ST.vol-.1));showUI();break;case'KeyM':e.preventDefault();tMute();showUI();break;case'KeyF':e.preventDefault();document.fullscreenElement?document.exitFullscreen():el.root.requestFullscreen();break;}});}
function bndEvt(){
const v=el.vid;
v.addEventListener('play',()=>{ST.started=true;ST.playing=true;ST.ended=false;el.pl2.innerHTML=IC.pause;el.cp.innerHTML=IC.pause;el.th.style.display='none';el.cp.classList.remove('ns');showUI();el.spin.classList.remove('ns');});
v.addEventListener('pause',()=>{ST.playing=false;if(!ST.ended){el.pl2.innerHTML=IC.play;el.cp.innerHTML=IC.play;}showUI();el.spin.classList.remove('ns');});
v.addEventListener('ended',()=>{const i=_pl.findIndex(x=>x.id===_eid);if(i<_pl.length-1)R.ep(_sid,_pl[i+1].id);else{ST.playing=false;ST.ended=true;el.pl2.innerHTML=IC.replay;el.cp.innerHTML=IC.replay;el.cp.classList.add('ns');showUI();}});
v.addEventListener('waiting',()=>el.spin.classList.add('ns'));
v.addEventListener('canplay',()=>el.spin.classList.remove('ns'));
v.addEventListener('loadedmetadata',()=>{tick();const rt=D.getResumeTime(_sid,_eid);if(rt>5&&rt<(v.duration-10)){v.currentTime=rt;nudge(`Resumed ${fT(rt)}`,'c');}});
el.pl2.onclick=e=>{e.stopPropagation();tPlay();};
el.cp.onclick=e=>{e.stopPropagation();tPlay();};
el.prev.onclick=e=>{e.stopPropagation();const i=_pl.findIndex(x=>x.id===_eid);if(i>0)R.ep(_sid,_pl[i-1].id);};
el.next.onclick=e=>{e.stopPropagation();const i=_pl.findIndex(x=>x.id===_eid);if(i<_pl.length-1)R.ep(_sid,_pl[i+1].id);};
el.vol.onclick=e=>{e.stopPropagation();tMute();};
el.vs.oninput=e=>setVol(parseFloat(e.target.value));
el.fs.onclick=e=>{e.stopPropagation();document.fullscreenElement?document.exitFullscreen():el.root.requestFullscreen();};
el.spb.onclick=e=>{e.stopPropagation();cMenus();sMn('s');};
el.lb.onclick=e=>{e.stopPropagation();cMenus();sMn('l');};
el.qb.onclick=e=>{e.stopPropagation();cMenus();sMn('q');};
el.plb.onclick=e=>{e.stopPropagation();sMn('p');};
el.plc.onclick=e=>{e.stopPropagation();sMn('p',false);};
...
el.st.addEventListener('mousemove',e=>{const f=fFrac(e);el.sh.textContent=fT(f*(el.vid.duration||0));el.sh.style.left=100*f+'%';el.sh.style.opacity='1';});
el.st.addEventListener('mouseleave',()=>el.sh.style.opacity='0');
...
el.ov.addEventListener('touchstart',e=>{if(!ST.started)return;const t=e.touches[0],now=Date.now();if(now-ST.lastTap<CFG.tapMs){clearTimeout(ST.holdTmr);const r=el.ov.getBoundingClientRect(),x=t.clientX-r.left;if(x<r.width/3)seek(-CFG.skip);else if(x>2*r.width/3)seek(CFG.skip);else tPlay();ST.lastTap=0;return;}ST.lastTap=now;ST.holdTmr=setTimeout(()=>{if(!el.vid.paused){el.vid.playbackRate=CFG.holdSpd;el.sh2.classList.add('ns');}},CFG.holdDly);},{passive:true});
el.ov.addEventListener('touchend',()=>{clearTimeout(ST.holdTmr);if(el.vid.playbackRate===CFG.holdSpd&&ST.spd!==CFG.holdSpd){el.vid.playbackRate=ST.spd;el.sh2.classList.remove('ns');}},{passive:true});
document.addEventListener('fullscreenchange',()=>{const f=!!document.fullscreenElement;el.fs.innerHTML=f?IC.exitFs:IC.fullscreen;el.root.classList.toggle('nrfs',f);});
...
el=glk();
el.pl2.innerHTML=IC.play;el.cp.innerHTML=IC.play;el.prev.innerHTML=IC.skipPrev;el.next.innerHTML=IC.skipNext;el.vol.innerHTML=IC.volume;el.fs.innerHTML=IC.fullscreen;el.plb.innerHTML=IC.playlist;el.plc.innerHTML=IC.close;
el.spb.innerHTML=IC.speed+'<span>1x</span>';el.lb.innerHTML=IC.language+'<span>EN</span>';el.qb.innerHTML=IC.quality+'<span>720p</span>';
bldSpd();bldPl();
const langs=Object.keys(show.tracks),fl=langs[0],fq=Object.keys(show.tracks[fl])[0];
loadTrk(fl,fq,false);bndEvt();updVol();el.cp.classList.add('ns');el.root.focus();if(ST.raf)cancelAnimationFrame(ST.raf);tick();
},
