...
/* ── Share ── */
function share(id,eid){const s=D.getShow(id);if(!s)return;const ep=eid?D.getEp(id,eid):null;const mv=!!s.isMovie;const title=mv?s.title:ep?`${s.title} · ${ep.title}`:s.title;const text=ep?ep.desc||s.desc:s.desc;const url=eid?`${location.origin}${location.pathname}#ep/${id}/${eid}`:`${location.origin}${location.pathname}#show/${id}`;if(navigator.share){navigator.share({title,text,url}).catch(()=>{});}else{navigator.clipboard?navigator.clipboard.writeText(url).then(()=>toast('Link copied!')).catch(()=>toast('Link: '+url)):toast('Link: '+url);}}
/* ── Dynamic metadata ── */
function setMeta(id,eid){const s=D.getShow(id);if(!s)return;const ep=eid?D.getEp(id,eid):null;const mv=!!s.isMovie;const title=ep&&!mv?`${ep.title} · ${s.title} — AnimeX`:`${s.title} — AnimeX`;const desc=ep?ep.desc||s.desc:s.desc;const img=ep?ep.thumb||s.thumb:s.hero||s.thumb;const url=eid?`${location.origin}${location.pathname}#ep/${id}/${eid}`:`${location.origin}${location.pathname}#show/${id}`;const _s=(sel,attr,val)=>{let el=document.querySelector(sel);if(!el){el=document.createElement('meta');if(attr==='name')el.setAttribute('name',sel.match(/name="([^"]+)"/)?.[1]||'');else el.setAttribute('property',sel.match(/property="([^"]+)"/)?.[1]||'');document.head.appendChild(el);}el.setAttribute('content',val);};document.title=title;_s('meta[name="description"]','name',desc||'');_s('meta[property="og:title"]','property',title);_s('meta[property="og:description"]','property',desc||'');_s('meta[property="og:image"]','property',img||'');_s('meta[property="og:url"]','property',url);_s('meta[property="og:type"]','property','video.other');_s('meta[name="twitter:card"]','name','summary_large_image');_s('meta[name="twitter:title"]','name',title);_s('meta[name="twitter:description"]','name',desc||'');_s('meta[name="twitter:image"]','name',img||'');}
...
return{home,search,profile,buildEps,tVM,toast,lI,obsL,_card,wlBtn,openPlFull,closePlFull,_plfsRnd,_plfsTab,share,openRating,_doRate,setMeta,
