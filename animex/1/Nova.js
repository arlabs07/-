...
let ST={lang:'en',qual:'720p',spd:1,vol:1,muted:false,playing:false,ended:false,started:false,uiTmr:null,uiFull:false,plOpen:false,chOpen:false,capOpen:false,langOpen:false,spdOpen:false,qualOpen:false,lastTap:0,drag:false,raf:null,holdTmr:null,fs:false};
let _sid=null,_eid=null,_show=null,_ep=null,_pl=[];
let _si=false;
const _prefKey=(sid,eid)=>`ax_pref_${sid}_${eid}`;
...
#nct{position:absolute;top:0;left:0;right:0;z-index:20;padding:14px 18px;display:flex;align-items:center;gap:6px;opacity:0;transform:translateY(-8px);transition:opacity .3s,transform .3s;pointer-events:none}
...
#nfst{color:#fff;font-size:15px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:55%;background:rgba(10,10,15,.7);backdrop-filter:blur(10px);padding:5px 12px;border-radius:8px;border:1px solid rgba(255,255,255,.1);opacity:0;visibility:hidden;transition:opacity .3s;flex:1}
#nr.nrfs #nfst{opacity:1;visibility:visible}
#nct-fs-btns{display:none;align-items:center;gap:4px;margin-left:auto}
#nr.nrfs #nct-fs-btns{display:flex}
...
.npich{font-size:10px;color:rgba(240,240,248,.4);font-variant-numeric:tabular-nums}
...
video::cue{font-size:1.1em;background:rgba(0,0,0,.75);color:#fff;border-radius:4px;padding:2px 6px}
...
function cMenus(){['s','l','q','p','ch','cap'].forEach(k=>sMn(k,false));}
function sMn(k,v){
const M={s:[el.spm,'spdOpen'],l:[el.lm,'langOpen'],q:[el.qm,'qualOpen'],p:[el.plp,'plOpen'],ch:[el.plp,'chOpen'],cap:[el.plp,'capOpen']};
if(!M[k]||!M[k][0])return;
const on=v!==undefined?v:!ST[M[k][1]];
if(on)Object.keys(M).filter(x=>x!==k).forEach(x=>{M[x][0]&&M[x][0].classList.remove('no');ST[M[x][1]]=false;});
M[k][0].classList.toggle('no',on);ST[M[k][1]]=on;
if(on&&k==='s')setTimeout(updSpd,50);
if(on&&k==='p'){bldPl();const npph=g('npph');if(npph){npph.innerHTML=`<span>Episodes</span><button class="nb" onclick="NV._sMn('p',false)">${IC.close}</button>`;}}
if(on&&k==='ch'){bldCh();const npph=g('npph');if(npph){npph.innerHTML=`<span>Chapters</span><button class="nb" onclick="NV._sMn('ch',false)">${IC.close}</button>`;}}
if(on&&k==='cap'){bldCap();const npph=g('npph');if(npph){npph.innerHTML=`<span>Subtitles</span><button class="nb" onclick="NV._sMn('cap',false)">${IC.close}</button>`;}}
}
...
function loadTrk(lk,qk,resume){const tr=(_ep&&_ep.tracks)||{};const langs=Object.keys(tr);if(!langs.length)return;if(!langs.includes(lk))lk=langs[0];ST.lang=lk;const quals=Object.keys(tr[lk]);if(!quals.includes(qk))qk=quals[0];ST.qual=qk;const src=tr[lk][qk],ct=resume?el.vid.currentTime:0,wp=!el.vid.paused;el.vid.src=src;el.vid.load();el.vid.currentTime=ct;ST.ended=false;if(wp&&resume)el.vid.play().catch(()=>{});if(el.th){el.th.src=_ep.thumb||'';el.th.style.display='block';}if(el.cth)el.cth.src=_ep.thumb||'';if(el.fst)el.fst.textContent=`${_show.title}${_ep.title?' · '+_ep.title:''}`;bldLng(langs);bldQual(quals);if(el.lb){el.lb.querySelector('span').textContent=lk.toUpperCase();el.lb.style.display=langs.length>1?'flex':'none';}if(el.qb){el.qb.querySelector('span').textContent=qk;el.qb.style.display=quals.length>1?'flex':'none';}_savePref();loadCaptions();}
function loadCaptions(){if(!el.vid)return;const caps=(_ep&&_ep.captions)||[];Array.from(el.vid.querySelectorAll('track')).forEach(t=>t.remove());caps.forEach(c=>{const t=document.createElement('track');t.kind='subtitles';t.label=c.label||c.lang||'';t.srclang=c.lang||'en';t.src=c.src;el.vid.appendChild(t);});if(el.vid.textTracks.length>0)el.vid.textTracks[0].mode='showing';}
...
function bldPl(){const list=g('nppls');if(!list)return;list.innerHTML='';_pl.forEach(ep=>{const li=document.createElement('li');li.className='nppi';li.innerHTML=`<div class="npit"><img src="${ep.thumb||''}"></div><div class="npim"><span class="npin">S${ep.s} E${ep.e} · ${ep.title}</span><span class="npis">${ep.dur||''}</span></div>`;li.classList.toggle('na',ep.id===_eid);li.onclick=()=>{sMn('p',false);R.ep(_sid,ep.id);};list.appendChild(li);});}
function bldCh(){const list=g('nppls');if(!list)return;const chs=(_ep&&_ep.chapters)||[];list.innerHTML='';if(!chs.length){list.innerHTML='<li style="padding:20px 16px;color:rgba(255,255,255,.4);font-size:13px;text-align:center">No chapters available</li>';return;}chs.forEach((ch,i)=>{const next=chs[i+1];const li=document.createElement('li');li.className='nppi';const cur=el.vid&&el.vid.currentTime>=ch.t&&(!next||el.vid.currentTime<next.t);li.classList.toggle('na',cur);li.innerHTML=`<div class="npim" style="flex-direction:row;align-items:center;gap:10px;width:100%"><span class="npich">${fT(ch.t)}</span><span class="npin" style="flex:1">${ch.title}</span></div>`;li.onclick=()=>{if(el.vid)el.vid.currentTime=ch.t;sMn('ch',false);nudge(ch.title,'c');};list.appendChild(li);});}
function bldCap(){const list=g('nppls');if(!list)return;const caps=(_ep&&_ep.captions)||[];list.innerHTML='';const addOff=document.createElement('li');addOff.className='nppi';const offAct=!el.vid||Array.from(el.vid.textTracks).every(t=>t.mode==='disabled'||t.mode==='hidden');addOff.classList.toggle('na',offAct);addOff.innerHTML=`<div class="npim"><span class="npin">${offAct?IC.check+' ':''}Off</span></div>`;addOff.onclick=()=>{if(el.vid)Array.from(el.vid.textTracks).forEach(t=>t.mode='hidden');sMn('cap',false);};list.appendChild(addOff);caps.forEach(c=>{const li=document.createElement('li');li.className='nppi';const act=el.vid&&Array.from(el.vid.textTracks).some(t=>t.srclang===c.lang&&t.mode==='showing');li.classList.toggle('na',act);li.innerHTML=`<div class="npim"><span class="npin">${act?IC.check+' ':''}${c.label||c.lang}</span></div>`;li.onclick=()=>{if(el.vid){Array.from(el.vid.textTracks).forEach(t=>t.mode='hidden');const trk=Array.from(el.vid.textTracks).find(t=>t.srclang===c.lang);if(trk)trk.mode='showing';}sMn('cap',false);};list.appendChild(li);});}
function bndEvt(){
const v=el.vid;
...
tick();
const rt=_loadPos();
if(rt>2&&rt<(v.duration-5)){v.currentTime=rt;nudge(`Resumed ${fT(rt)}`,'c');v.play().catch(()=>{});}
});
...
if(el.chb)el.chb.onclick=e=>{e.stopPropagation();sMn('ch');};
if(el.capb)el.capb.onclick=e=>{e.stopPropagation();sMn('cap');};
const ppc=g('nppc');if(ppc)ppc.onclick=e=>{e.stopPropagation();cMenus();};
...
document.addEventListener('fullscreenchange',()=>{ST.fs=!!document.fullscreenElement;if(el.fs)el.fs.innerHTML=ST.fs?IC.exitFs:IC.fullscreen;if(el.root)el.root.classList.toggle('nrfs',ST.fs);});
document.addEventListener('click',e=>{if(!e.target.closest('#nspb')&&!e.target.closest('#nspm'))sMn('s',false);if(!e.target.closest('#nlb')&&!e.target.closest('#nlm'))sMn('l',false);if(!e.target.closest('#nqb')&&!e.target.closest('#nqm'))sMn('q',false);if((ST.plOpen||ST.chOpen||ST.capOpen)&&!e.target.closest('#npp')&&!e.target.closest('#nplb')&&!e.target.closest('#nchb')&&!e.target.closest('#ncapb'))cMenus();});
}
return{
init(sid,eid,show,ep){
injectCSS();_sid=sid;_eid=eid;_show=show;_ep=ep;_pl=show.episodes||[];
ST={lang:'en',qual:'720p',spd:1,vol:1,muted:false,playing:false,ended:false,started:false,uiTmr:null,uiFull:false,plOpen:false,chOpen:false,capOpen:false,langOpen:false,spdOpen:false,qualOpen:false,lastTap:0,drag:false,raf:null,holdTmr:null,fs:false};
el=glk();
if(el.pl2)el.pl2.innerHTML=IC.play;if(el.cp)el.cp.innerHTML=IC.play;if(el.prev)el.prev.innerHTML=IC.skipPrev;if(el.next)el.next.innerHTML=IC.skipNext;if(el.vol)el.vol.innerHTML=IC.volume;if(el.fs)el.fs.innerHTML=IC.fullscreen;if(el.plb)el.plb.innerHTML=IC.playlist;
if(el.chb)el.chb.innerHTML=IC.chapters;
if(el.capb)el.capb.innerHTML=IC.captions;
...
const langs=Object.keys(ep.tracks);
const fl=pref&&langs.includes(pref.lang)?pref.lang:langs[0];
const quals=Object.keys(ep.tracks[fl]);
const fq=pref&&quals.includes(pref.qual)?pref.qual:quals[0];
loadTrk(fl,fq,false);
}
