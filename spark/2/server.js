...
const Server = (() => {
  ...
  const updatePresence = async () => {
    const me = _currentUser; if (!me) return;
    const profile = _currentProfile; if (!profile?.id) return;
    await db('spark_profiles').patch(profile.id, {
      last_seen: new Date().toISOString(), is_online: true,
    }).catch(() => {});
  };

  const getUserPresence = async (userId) => {
    if (!userId) return { online: false, last_seen: null };
    try {
      const prof = await getProfile(userId); const data = prof?.data;
      if (!data) return { online: false, last_seen: null };
      const online = data.last_seen && (Date.now() - new Date(data.last_seen).getTime()) < 2 * 60 * 1000;
      return { online: !!online, last_seen: data.last_seen || null };
    } catch { return { online: false, last_seen: null }; }
  };

  /* ── Read Receipts ────────────────────────────────────────── */
  const markMessageRead = async (chatId, msgTime, userId) => {
    if (!chatId || !msgTime || !userId) return;
    const rec = await getChatById(chatId); if (!rec) return;
    const messages = (rec.data?.messages || []).map(m => {
      if (m.time !== msgTime) return m;
      const rb = m.read_by || [];
      if (rb.includes(userId)) return m;
      return { ...m, read_by: [...rb, userId] };
    });
    await db('spark_chats').patch(chatId, { messages }).catch(() => {});
  };

  /* ── Invite Links ─────────────────────────────────────────── */

  /**
   * Generate a unique invite token for the current user and store it
   * in their profile. Returns the full shareable URL.
   */
  const generateInviteLink = async () => {
    const me      = _currentUser; if (!me) throw new Error('Not logged in');
    const profile = _currentProfile; if (!profile?.id) throw new Error('No profile');

    // Create a token: userId + random suffix
    const token = me.id.slice(0, 8) + '_' + Math.random().toString(36).slice(2, 10);
    await db('spark_profiles').patch(profile.id, { invite_token: token });

    // Store locally too
    if (_currentProfile?.data) _currentProfile.data.invite_token = token;

    // Build URL — uses window.location so it works on any domain
    const base = window.location.origin + window.location.pathname.replace(/index\.html$/, '');
    return `${base}#invite/${token}`;
  };

  /**
   * Resolve an invite token to a user profile.
   * Returns the profile record or null.
   */
  const resolveInviteToken = async (token) => {
    if (!token) return null;
    try {
      const res = await db('spark_profiles').list({ search: token, limit: '50' });
      return (res.data || []).find(r => r.data?.invite_token === token) || null;
    } catch { return null; }
  };

  /**
   * Accept an invite: start a DM with the inviting user.
   * Called after login when the hash is #invite/TOKEN.
   * Returns the chat record id.
   */
  const acceptInvite = async (token) => {
    const inviter = await resolveInviteToken(token); if (!inviter) throw new Error('Invalid invite link');
    const me      = _currentUser; const myProfile = _currentProfile;
    if (!me || !myProfile) throw new Error('Not logged in');

    const inviterId = inviter.data.user_id;
    if (inviterId === me.id) throw new Error('Cannot invite yourself');

    // Find or create DM
    let chatRec = await findDirectChat(me.id, inviterId);
    if (!chatRec) {
      chatRec = await createDirectChat(
        ...
        { user_id: inviterId, display_name: inviter.data.display_name, username: inviter.data.username, avatar_url: inviter.data.avatar_url || '' }
      );
    }
    return chatRec?.id || null;
  };
  ...
  return {
    ...
    updatePresence, getUserPresence, markMessageRead,
    generateInviteLink, resolveInviteToken, acceptInvite,
    db, storage
  };
})();
