/**
 * chat.js — ChatWindow v8
 *
 * Improvements:
 *  - Enter key = new line (Ctrl/Cmd+Enter or send button to send)
 *  - Delta sync: only appends new messages to DOM, patches edits/reactions in-place
 *  - Read receipts: blue tick when other user views the message
 *  - Online presence: "Online" or "Last seen X" in header sub-line
 *  - Redesigned bubbles, polls, quiz (card-based)
 *  - Image/file/PDF fullscreen overlay with toolbar (download, share, close)
 *  - Disappearing: 365 days replaces "never"
...
const ChatWindow = (() => {
  ...
  const open = async (chatId, overlayEl, opts = {}) => {
    ...
    // Start presence ping + load partner presence
  ...
  const _buildUI = () => {
    ...
    if (canMsg) {
      ...
      ta.addEventListener('keydown', e => {
        ...
        // plain Enter → let default newline happen (no send)
      });
    ...
    // Full render on first load, then use delta from here on
    _renderMessages();
  ...
  /* ══════════════════════════════════════════════════════════
     DELTA POLL  — only appends new, patches existing
     ══════════════════════════════════════════════════════════ */
  ...
  const _pollFetch = async () => {
    ...
    // Merge optimistic into full list
    ...
    // Patch updated messages — reactions and blue ticks
    updatedMsgs.forEach(msg => {
      SyncManager.patchMessage(msg.time, (row) => {
        ...
        // Blue tick: update if this is our sent message and partner has read it
  ...
  /* ══════════════════════════════════════════════════════════
     RENDER  (full initial render only)
     ══════════════════════════════════════════════════════════ */
  ...
  const _msgRowHtml = (msg, showName = false) => {
    ...
    if (isDeleted) {
      ...
      // Text message — decrypt if encrypted
    ...
    const tickCls  = isFailed ? '' : isOpt ? 'pending' : otherRead ? 'read' : 'delivered';
  ...
  /** Bind all interactive events to a message row element. */
  ...
  const _setReply = (msg) => {
    ...
    document.getElementById('reply-bar-sender').textContent =
      msg.display_name || msg.username || 'Message';
  ...
  const _optimisticSend = async (msg) => {
    ...
    try {
      ...
      _renderMessages();   // full re-render to get correct state after send
    } catch {
  ...
  /**
   * Optimistic send where displayMsg (plain) is shown immediately,
   * but serverMsg (encrypted) is what actually gets sent to the server.
   */
  const _optimisticSendEncrypted = async (displayMsg, serverMsg) => {
    ...
    try {
      ...
      // Replace local messages with server truth, decrypting encrypted ones
  ...
  const _sendText = async (text) => {
    ...
    // Keep local copy unencrypted for immediate display
  ...
  const _votePoll = async (msgTime, optIdx, pollType='poll') => {
    ...
    // Patch DOM: find and REPLACE just the card element — no innerHTML splitting
    SyncManager.patchMessage(msgTime, row => {
      ...
      if (newCard) {
        ...
        // Re-bind vote buttons on the new card
        row.querySelectorAll('.poll-opt-btn').forEach(btn =>
  ...
  const _pollBubbleHtml = (msg, myId, pollType='poll') => {
    ...
    if (isQuiz) {
      ...
      return `<div class="quiz-card">
        <div class="quiz-card-header">
          <span class="quiz-tag">
            <span class="material-icons-round" style="font-size:11px;vertical-align:middle">quiz</span> Quiz
          </span>
          ${hasVoted ? `<span class="quiz-answered-badge">
            <span class="material-icons-round">check_circle</span> Answered
          </span>` : ''}
        </div>
    ...
    return `<div class="poll-card">
      <div class="poll-card-header">
        ...
        <span class="poll-card-tag">Poll</span>
      </div>
    ...
    const close = App.showModal(`
      <div style="padding:20px 0 30px">
        ...
        <p style="font-size:13px;color:var(--text-3);padding:12px 20px;line-height:1.5">
          Messages older than the selected duration are automatically hidden.
        </p>
  ...
  const _showMediaFS = (url, type, fileName) => {
    ...
    el.innerHTML = `
      ...
      <div class="media-fs-toolbar">
        <button class="media-fs-back" id="mfs-close">
          <span class="material-icons-round">arrow_back</span>
        </button>
        ...
        <!-- Zoom badge centered in toolbar -->
        <div class="media-fs-zoom-badge" id="mfs-zoom-badge">1×</div>
        ${type !== 'video' ? `<button class="media-fs-action" id="mfs-share" title="Share">
          <span class="material-icons-round">share</span>
        </button>` : ''}
        <button class="media-fs-action" id="mfs-download" title="Download">
          <span class="material-icons-round">download</span>
        </button>
      </div>

      <!-- Content -->
      <div class="media-fs-content" id="mfs-content">
        ${contentHtml}
      </div>

      <!-- Bottom caption bar -->
      <div class="media-fs-bottom">
        <div class="media-fs-caption">${_esc(fileName || '')}</div>
      </div>`;
    ...
    document.getElementById('mfs-share')?.addEventListener('click', () => {
      if (navigator.share) {
        navigator.share({ title: fileName || 'Shared file', url }).catch(() => {});
      } else {
        navigator.clipboard?.writeText(url).then(() => App.showToast('Link copied!')).catch(() => {});
      }
    });
    ...
    if (type === 'image') {
      ...
      el.addEventListener('touchmove', e => {
        ...
        const badge = document.getElementById('mfs-zoom-badge');
        if (badge) {
      ...
      el.addEventListener('touchend', () => {
        setTimeout(() => {
          document.getElementById('mfs-zoom-badge')?.classList.remove('show');
        }, 1500);
      });
      ...
      <div class="gi-sheet">
        <div class="gi-header-block">
          ...
          <div class="gi-count">
            ...
            <span class="material-icons-round" style="font-size:13px">${isPublic?'public':'lock'}</span>${isPublic?'Public':'Private'}
            ${adminOnly?`<span style="margin-left:6px;font-size:11px;padding:2px 7px;border-radius:99px;background:rgba(240,160,48,0.15);color:var(--warning);font-weight:700">Admin only</span>`:''}
          </div>
        ...
        <div class="ctx-action" id="gi-disappear" style="padding:12px 0">
          <span class="material-icons-round" style="color:var(--warning)">timer</span> Disappearing Messages
  ...
  // _showEditGroup, _showConvertToGroup, _showAddMember — identical pattern, kept compact:
