...
const App = (() => {
  ...
  const navigate = (hash) => {
    ...
    if (page === 'invite' && param) {
      if (!_isAuth) {
        // Save token in sessionStorage → handled after login
        try { sessionStorage.setItem('spark_pending_invite', param); } catch {}
        window.location.hash = '#login';
        return;
      }
      // Already logged in → accept invite immediately
      _handleInvite(param);
      return;
    }
  ...
  /** Handle invite token — accept and open the resulting DM. */
  ...
  /** Check for a pending invite after login completes. */
