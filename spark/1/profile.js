...
const ProfilePage = (() => {
  ...
  const _render = async (profileRec) => {
      ...
      <div class="profile-scroll" id="prof-scroll">
        ...
        <div class="profile-header-card">
          ...
          <div class="profile-action-row">
            ...
            <button class="btn-ghost" id="btn-invite-link" title="Share invite link">
              <span class="material-icons-round">link</span> Invite
            </button>
    ...
    // Invite link
    document.getElementById('btn-invite-link').onclick = _showInviteSheet;
  ...
  /* ─── INVITE LINK ─────────────────────────────────────────── */
  const _showInviteSheet = async () => {
    let _link = '';

    App.showModal(`
      <div style="padding:28px 20px 36px;display:flex;flex-direction:column;align-items:center;gap:16px;text-align:center">
        <div style="width:64px;height:64px;border-radius:50%;background:var(--accent-dim);
          display:flex;align-items:center;justify-content:center">
          <span class="material-icons-round" style="font-size:28px;color:var(--accent)">link</span>
        </div>
        <h3 style="font-size:18px;font-weight:800;color:var(--text-1)">Your Invite Link</h3>
        <p style="font-size:13px;color:var(--text-3);line-height:1.6;max-width:280px">
          Share this link. Anyone who taps it will be taken to Spark and immediately start a chat with you — even if they need to create an account first.
        </p>

        <div id="invite-box" style="width:100%;background:var(--bg-3);border:1.5px solid var(--border-light);
          border-radius:12px;padding:12px 14px;display:flex;align-items:center;gap:10px;cursor:pointer">
          <span class="material-icons-round" style="color:var(--accent);flex-shrink:0;font-size:18px">link</span>
          <div id="invite-link-text" style="flex:1;min-width:0;font-size:12px;color:var(--text-2);
            white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:left">
            <span class="spinner" style="width:14px;height:14px;border-width:2px;display:inline-block"></span>
            Generating...
          </div>
          <span class="material-icons-round" style="color:var(--text-3);flex-shrink:0;font-size:18px">content_copy</span>
        </div>

        <div style="display:flex;gap:10px;width:100%">
          <button class="btn-ghost" id="invite-copy-btn" style="flex:1;display:flex;align-items:center;justify-content:center;gap:6px">
            <span class="material-icons-round" style="font-size:16px">content_copy</span> Copy
          </button>
          <button class="btn-primary" id="invite-share-btn" style="flex:1;display:flex;align-items:center;justify-content:center;gap:6px">
            <span class="material-icons-round" style="font-size:16px">share</span> Share
          </button>
        </div>

        <button id="invite-regen-btn" style="background:none;border:none;cursor:pointer;
          font-size:12px;color:var(--text-3);display:flex;align-items:center;gap:4px;font-family:var(--font);padding:6px">
          <span class="material-icons-round" style="font-size:14px">refresh</span>
          Generate a new link (invalidates old one)
        </button>

        <p style="font-size:11px;color:var(--text-3);line-height:1.5">
          The link only grants access to chat with you — not your account.
        </p>
      </div>`);

    const _gen = async () => {
      const textEl = document.getElementById('invite-link-text');
      if (textEl) textEl.innerHTML = `<span class="spinner" style="width:14px;height:14px;border-width:2px;display:inline-block"></span> Generating...`;
      try {
    ...
    document.getElementById('invite-box')?.addEventListener('click', _doCopy);
    document.getElementById('invite-copy-btn')?.addEventListener('click', _doCopy);

    document.getElementById('invite-share-btn')?.addEventListener('click', () => {
      if (!_link) return;
      if (navigator.share) {
        navigator.share({ title: 'Chat with me on Spark', text: 'Tap to start a chat with me on Spark!', url: _link })
          .catch(() => {});
      } else { _doCopy(); }
    });

    document.getElementById('invite-regen-btn')?.addEventListener('click', _gen);
  };
