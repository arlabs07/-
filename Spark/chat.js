/**
 * chat.js — ChatWindow Module v4
 *
 * Performance architecture:
 *  • _msgHash   — JSON-hash guards: skips re-render if messages unchanged
 *  • Local state — _msgs array kept in JS, synced with server in background
 *  • Optimistic  — messages appear instantly; confirmed/replaced on poll
 *  • Poll diff   — 4s interval, only re-renders when hash differs
 *  • Canvas pool — reused via Server (server.js pools canvases)
 *
 * Features:
 *  • Algorithm 1 image compression before every upload
 *  • Attach overlay: Image · File · Camera (coming soon) · Poll (coming soon)
 *  • File bubbles: PDF 🔴 · DOC 🔵 · XLS 🟢 · Generic ⚪
 *  • Link preview: auto-detect URLs, show domain + favicon chip
 *  • 5 emoji reactions with long-press context menu
 *  • Edit / Delete (soft) messages
 *  • Convert direct → group, add/remove members
 *  • Edit group (name, description, avatar, color, privacy)
 *  • Private 🔒 / Public 🌐 badge in header
 *  • Image fullscreen viewer
 *  • Full-screen overlay — hides App header + bottom nav
 */
...
const ChatWindow = (() => {
  ...
  let _chatId     = null;
  let _chatRec    = null;
  let _msgs       = [];        // local message array (source of truth)
  let _msgHash    = '';        // JSON.stringify(_msgs) — render guard
  let _container  = null;
  let _pollTimer  = null;
  let _onClose    = null;
  let _isNew      = false;
  let _attachOpen = false;
  let _raf        = null;      // rAF handle for scroll
  ...
  const open = async (chatId, overlayEl, opts = {}) => {
    if (!chatId || chatId === 'undefined') {
      App.showToast('Cannot open chat — invalid ID.', 'error');
      return;
    }

    close();   // teardown any previous
    ...
    _container.innerHTML = _loadingHTML();
    ...
    if (_isNew && !_msgs.length) {
      setTimeout(() => _sendText('Hi 👋'), 350);
    }
  ...
  const close = () => {
    ...
    _pollTimer = _raf = null;
    _chatId = _chatRec = null;
    _msgs = []; _msgHash = '';
    _isNew = false; _attachOpen = false;
  };
  ...
  const _buildUI = () => {
    ...
    if (isGroup) {
      const color   = d.color || '#0095f6';
      const initial = (d.name || 'G')[0].toUpperCase();
      const count   = d.member_count || (d.participants || []).length;
      hName   = d.name || 'Group';
      hSub    = `${count} member${count !== 1 ? 's' : ''}`;
      hAvHtml = `<div class="cw-group-av" id="cw-av-btn"
        style="background:${color}22;color:${color}">
        ${d.avatar_url ? `<img src="${_attr(d.avatar_url)}">` : initial}
      </div>`;
      const pub = d.is_public !== false;
      privBadge = `<span class="privacy-badge ${pub ? 'public' : 'private'}">
        <span class="material-icons-round">${pub ? 'public' : 'lock'}</span>${pub ? 'Public' : 'Private'}
      </span>`;
    } else {
    ...
    _container.innerHTML = `
      <div class="chat-fullscreen">
        ...
        <div class="cw-input-bar" id="cw-input-bar">
          ...
          <!-- Attach overlay (slides up) -->
          <div class="attach-overlay" id="attach-overlay">
            <div class="attach-grid">
              <div class="attach-item" data-attach="image">
                <div class="attach-icon-wrap purple">
                  <span class="material-icons-round">image</span>
                </div>
                <span class="attach-label">Photo</span>
              </div>
              <div class="attach-item" data-attach="file">
                <div class="attach-icon-wrap blue">
                  <span class="material-icons-round">attach_file</span>
                </div>
                <span class="attach-label">Document</span>
              </div>
              <div class="attach-item" data-attach="camera" disabled class="disabled">
                <div class="attach-icon-wrap gray">
                  <span class="material-icons-round">photo_camera</span>
                </div>
                <span class="attach-label" style="color:var(--text-3)">Camera</span>
              </div>
              <div class="attach-item" data-attach="poll" class="disabled">
                <div class="attach-icon-wrap gray">
                  <span class="material-icons-round">poll</span>
                </div>
                <span class="attach-label" style="color:var(--text-3)">Poll</span>
              </div>
            </div>
          </div>
          ...
          <!-- Insert button -->
          <button class="cw-insert-btn" id="cw-insert" title="Attach">
  ...
  /* ══════════════════════════════════════════════════════════
     POLLING  (hash-guarded, zero wasted re-renders)
     ══════════════════════════════════════════════════════════ */
  const _poll = async () => {
    if (!_chatId) return;
    ...
    if (newHash === _msgHash) return;   // ← zero DOM work if unchanged

    // Merge: keep local optimistic messages not yet on server,
    // but replace with server truth where times match.
  ...
  const _renderMessages = () => {
    ...
    _msgs.forEach(msg => {
      ...
      const optClass  = isOpt ? ' optimistic' : '';
      const failClass = isFailed ? ' failed' : '';
      ...
      const failRetry = isFailed
        ? `<span class="retry-btn" data-t="${_attr(msg.time)}">Retry</span>` : '';

      html += `
        <div class="msg-row ${cls}${optClass}${failClass}"
    ...
    // Bubble long-press (for non-system, non-deleted)
    ...
    area.querySelectorAll('.reaction-pill').forEach(pill =>
      pill.addEventListener('click', e => {
        e.stopPropagation();
        _toggleReaction(pill.dataset.t, pill.dataset.e);
      })
    );
    ...
    area.querySelectorAll('.retry-btn').forEach(btn =>
      btn.addEventListener('click', () => {
        const msg = _findMsg(btn.dataset.t);
        if (msg) _retryFailed(msg);
      })
    );
  ...
  /* ══════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════ */
  const _toggleAttach = () => {
    _attachOpen ? _closeAttach() : _openAttach();
  };
  const _openAttach = () => {
  ...
  /* ══════════════════════════════════════════════════════════
     SEND — OPTIMISTIC PATTERN
     ══════════════════════════════════════════════════════════ */

  const _myMeta = () => {
    const me  = Server.currentUser;
    const mpD = Server.currentProfile?.data || {};
    return {
      sender_id:    me?.id || '',
      username:     mpD.username     || '',
      display_name: mpD.display_name || me?.display_name || 'Me',
    };
  };

  /** Append optimistically, then confirm with server. */
  const _optimisticSend = async (msg) => {
    ...
    try {
      ...
      // Replace local msgs with server truth (removes _optimistic flag)
      _msgs    = serverMsgs;
      _msgHash = JSON.stringify(_msgs);
      _renderMessages();
    } catch {
      // Mark as failed
      _msgs = _msgs.map(m =>
        m === tempMsg ? { ...m, _optimistic: false, _failed: true } : m
      );
      _msgHash = JSON.stringify(_msgs);
      _renderMessages();
    }
  ...
  const _retryFailed = async (msg) => {
    // Remove failed version, re-send
    _msgs = _msgs.filter(m => m !== msg && m.time !== msg.time);
    const cleanMsg = { ...msg };
    delete cleanMsg._failed; delete cleanMsg._optimistic;
  ...
  /* ── Send image (Algorithm 1 compress → upload → optimistic) ── */
  const _sendImageFile = async (file) => {
    App.showToast('Compressing & uploading…');
    try {
      // Algorithm 1: contrast/saturate/brightness → WebP 0.65 (fallback 0.45)
      const url = await Server.uploadCompressedImage(file, 'spark_chat_imgs');
      if (!url) throw new Error('No URL returned');

      const msg = { ..._myMeta(), message: url, time: _now(), msg_type: 'image' };
      await _optimisticSend(msg);
    } catch {
      App.showToast('Image upload failed', 'error');
    }
  };
  ...
  const _sendGenericFile = async (file) => {
    ...
    try {
      const fileData = await Server.uploadFile(file, 'spark_files');
      ...
      App.showToast('File upload failed', 'error');
    }
  };
  ...
  const _toggleReaction = async (msgTime, emoji) => {
    ...
    Server.addReaction(_chatId, msgTime, emoji, me.id)
      .then(updated => { if (updated) { _msgs = updated; _msgHash = JSON.stringify(updated); }})
      .catch(() => {});
  };

  const _reactionsHtml = (msg, myId) => {
    const r    = msg.reactions || {};
    const keys = Object.keys(r).filter(e => (r[e] || []).length > 0);
    if (!keys.length) return '';
    const pills = keys.map(e => {
      const cnt  = (r[e] || []).length;
      const mine = (r[e] || []).includes(myId);
      return `<button class="reaction-pill ${mine ? 'mine' : ''}"
        data-t="${_attr(msg.time)}" data-e="${e}">
        ${e} <span class="r-count">${cnt}</span>
      </button>`;
    }).join('');
  ...
  const _showCtxMenu = (msg) => {
    ...
    const close = App.showModal(`
      <div class="ctx-menu-wrap">
        ...
        ${msg.msg_type === 'text' ? `
        <div class="ctx-action" id="ctx-copy">
          <span class="material-icons-round">content_copy</span> Copy
        </div>` : ''}
        ${isOwn && msg.msg_type === 'text' ? `
        <div class="ctx-action" id="ctx-edit">
          <span class="material-icons-round">edit</span> Edit Message
        </div>` : ''}
        ${msg.msg_type === 'image' ? `
        <div class="ctx-action" id="ctx-save">
          <span class="material-icons-round">download</span> Save Image
        </div>` : ''}
        ${isOwn ? `
        <div class="ctx-action danger" id="ctx-del">
    ...
    document.getElementById('ctx-copy')?.addEventListener('click', () => {
      close(); navigator.clipboard?.writeText(msg.message).catch(() => {});
    ...
    document.getElementById('ctx-save')?.addEventListener('click', () => {
      close();
      const a = document.createElement('a'); a.href = msg.message; a.download = 'image.webp'; a.click();
    });
  ...
  const _startEdit = (msg) => {
    ...
    document.getElementById('edit-save')?.addEventListener('click', async () => {
      ...
      Server.editMessage(_chatId, msg.time, t).then(u => { if (u) { _msgs = u; _msgHash = JSON.stringify(u); }}).catch(() => {});
    });
  };
  ...
  const _confirmDelete = (msg) => {
    ...
    document.getElementById('del-confirm')?.addEventListener('click', async () => {
      ...
      _msgHash = JSON.stringify(_msgs); _renderMessages();
      Server.deleteMessage(_chatId, msg.time).then(u => { if (u) { _msgs = u; _msgHash = JSON.stringify(u); }}).catch(() => {});
    });
  };

  /* ══════════════════════════════════════════════════════════
     FILE + LINK PREVIEW RENDERERS
     ══════════════════════════════════════════════════════════ */

  const _fileBubbleHtml = (messageStr, isSent) => {
    const data = _safeJson(messageStr);
    if (!data) return _esc(messageStr);

    const ext   = (data.name || '').split('.').pop().toLowerCase();
    const cls   = ext === 'pdf' ? 'pdf' : ['doc','docx'].includes(ext) ? 'doc'
      : ['xls','xlsx','csv'].includes(ext) ? 'sheet' : 'generic';
    const icon  = cls === 'pdf' ? 'picture_as_pdf'
      : cls === 'doc'   ? 'description'
      : cls === 'sheet' ? 'table_chart' : 'insert_drive_file';
    ...
    return `<div class="file-bubble ${cls}">
      ...
      <a href="${_attr(data.url)}" download="${_attr(data.name)}" class="file-dl"
  ...
  const _extractUrl = (text) => {
    if (!text || typeof text !== 'string') return null;
    const m = text.match(URL_RE);
    return m ? m[0] : null;
  };

  const _linkPreviewHtml = (url, isSent) => {
    let domain = '';
    ...
    return `<a class="link-preview" href="${_attr(url)}" target="_blank" rel="noopener noreferrer">
      <img class="lp-icon" src="${favicon}" loading="lazy" alt=""
        onerror="this.style.display='none'">
      <div class="lp-info">
        <div class="lp-domain">${_esc(domain)}</div>
        <div class="lp-url">${_esc(display)}</div>
      </div>
    ...
    App.showModal(`
      <div style="padding:0 0 20px">
        ...
        <div class="ctx-action" id="dm-conv">
          <span class="material-icons-round" style="color:var(--accent)">group_add</span>
  ...
  const _showConvertToGroup = () => {
    ...
    const close = App.showModal(`
      <div class="edit-group-sheet">
        ...
        <div>
          <label class="auth-label" style="display:block;margin-bottom:6px">Group Name *</label>
          <input id="ctg-name" class="input-field" type="text" placeholder="Team Spark…" maxlength="60">
        </div>
        <div>
          <label class="auth-label" style="display:block;margin-bottom:8px">Add Members</label>
        ...
        <button class="eg-save-btn" id="ctg-ok">
          <span class="material-icons-round" style="font-size:18px">group_add</span> Create Group
        </button>
      </div>
    `);

    const renderChips = () => {
      const c = document.getElementById('ctg-chips');
      if (!c) return;
      c.innerHTML = added.map((m, i) => `
        <div class="member-chip">${_esc(m.display_name)}
          <span class="material-icons-round chip-x" data-i="${i}">close</span>
        </div>`).join('');
      c.querySelectorAll('.chip-x').forEach(x =>
        x.addEventListener('click', () => { added.splice(+x.dataset.i, 1); renderChips(); })
      );
    };
    ...
    document.getElementById('ctg-sr').addEventListener('input', (e) => {
      ...
      const res = document.getElementById('ctg-res');
      if (!q) { res.style.display = 'none'; return; }
      res.style.display = 'block';
      res.innerHTML = `<div style="padding:10px;font-size:13px;color:var(--text-3)">Searching…</div>`;
      srT = setTimeout(async () => {
        const found = await Server.getProfileByUsername(q);
        const me    = Server.currentUser;
        const ex    = _chatRec.data.participants || [];
        if (!found || found.data.user_id === me?.id || ex.includes(found.data.user_id)
          || added.some(m => m.user_id === found.data.user_id)) {
          res.innerHTML = `<div style="padding:10px;font-size:13px;color:var(--text-3)">${!found ? 'No user found.' : 'Already in chat.'}</div>`;
          return;
        }
        const fd = found.data;
        res.innerHTML = `<div class="inline-result-item" id="ctg-hit">
    ...
    document.getElementById('ctg-ok').onclick = async () => {
      const name  = document.getElementById('ctg-name').value.trim();
      const errEl = document.getElementById('ctg-err');
      errEl.classList.remove('visible');
      if (!name) { errEl.textContent = 'Name required.'; errEl.classList.add('visible'); return; }
      const btn = document.getElementById('ctg-ok');
      ...
      try {
        ...
        await Server.sendChatMessage(_chatId, { sender_id: 'system', username: '', display_name: '',
          message: `Chat converted to group: "${name}"`, time: _now(), msg_type: 'system' });
  ...
  const _showGroupInfo = () => {
    ...
    const memberRows = (d.participants || []).map(uid => {
      ...
      const isAdm = uid === d.created_by;
      return `<div class="gi-member">
        ${App.avatar(m.avatar_url, m.display_name, 'av-sm')}
        ...
        ${isCreator && !isMe ? `<div class="gi-remove" data-uid="${uid}">
          <span class="material-icons-round">person_remove</span></div>` : ''}
      </div>`;
    }).join('');
    ...
    document.querySelectorAll('.gi-remove').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          await Server.sendChatMessage(_chatId, { sender_id: 'system', username: '', display_name: '',
            message: `${meta.display_name || 'A member'} was removed.`, time: _now(), msg_type: 'system' });
    ...
    document.getElementById('gi-leave').addEventListener('click', async () => {
      ...
      await Server.sendChatMessage(_chatId, { sender_id: 'system', username: '', display_name: '',
        message: `${Server.currentProfile?.data.display_name || 'Someone'} left.`, time: _now(), msg_type: 'system' }).catch(() => {});
  ...
  const _showEditGroup = () => {
    ...
    let   selPublic = d.is_public !== false;
    let   avFile    = null;

    const dots = COLORS.map(c =>
      `<div class="color-dot ${c === selColor ? 'sel' : ''}" data-c="${c}" style="background:${c}"></div>`
    ).join('');
    ...
    const close = App.showModal(`
      <div class="edit-group-sheet">
        ...
        <div class="eg-av-pick">
          <label class="eg-av-btn" id="eg-av-lbl">
            ...
            <input type="file" accept="image/*" id="eg-av-in" style="display:none">
          </label>
        </div>
        <div>
          <label class="auth-label" style="display:block;margin-bottom:6px">Name *</label>
          <input id="eg-name" class="input-field" type="text" value="${_attr(d.name || '')}" maxlength="60">
        </div>
        <div>
          <label class="auth-label" style="display:block;margin-bottom:6px">Description</label>
          <textarea id="eg-desc" class="input-field" rows="2" style="resize:none"
            placeholder="What's this group about?">${_attr(d.description || '')}</textarea>
        </div>
        <div>
          <label class="auth-label" style="display:block;margin-bottom:8px">Color</label>
          <div class="color-row" id="eg-colors">${dots}</div>
        </div>
        <div>
          <label class="auth-label" style="display:block;margin-bottom:8px">Privacy</label>
          <div class="privacy-toggle">
            <button class="privacy-opt ${selPublic ? 'active' : ''}" data-v="true">
              <span class="material-icons-round">public</span>Public
            </button>
            <button class="privacy-opt ${!selPublic ? 'active' : ''}" data-v="false">
              <span class="material-icons-round">lock</span>Private
            </button>
          </div>
        </div>
        <div id="eg-err" class="auth-error"></div>
        <button class="eg-save-btn" id="eg-save">
    ...
    document.getElementById('eg-av-in').addEventListener('change', e => {
      ...
      rd.onload = ev => {
        const lbl = document.getElementById('eg-av-lbl');
        if (lbl) lbl.innerHTML = `<img src="${ev.target.result}" style="border-radius:14px;position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`;
      };
    ...
    document.getElementById('eg-save').onclick = async () => {
      ...
      try {
        ...
        if (avFile) {
          ...
          const url = await Server.uploadCompressedImage(avFile, 'spark_comm_avatars');
          if (url) avUrl = url;
        }
        await Server.updateCommunity(_chatId, { name, description: desc, color: selColor, avatar_url: avUrl, is_public: selPublic });
        await Server.sendChatMessage(_chatId, { sender_id: 'system', username: '', display_name: '',
          message: `Group updated: "${name}"`, time: _now(), msg_type: 'system' });
        _chatRec = await Server.getChatById(_chatId);
  ...
  const _showAddMember = () => {
    ...
    document.getElementById('am-sr').addEventListener('input', e => {
      clearTimeout(t);
      const q = e.target.value.trim();
      const res = document.getElementById('am-res');
      if (!q) { res.style.display = 'none'; return; }
      res.style.display = 'block';
      res.innerHTML = `<div style="padding:10px;font-size:13px;color:var(--text-3)">Searching…</div>`;
      t = setTimeout(async () => {
        const found = await Server.getProfileByUsername(q);
        const parts = _chatRec?.data?.participants || [];
        ...
        const fd = found.data;
        res.innerHTML = `<div class="inline-result-item" id="am-hit">
          ${App.avatar(fd.avatar_url, fd.display_name, 'av-sm')}
        ...
        document.getElementById('am-hit').querySelector('button').addEventListener('click', async () => {
          ...
          try {
            ...
            await Server.sendChatMessage(_chatId, { sender_id: 'system', username: '', display_name: '',
              message: `${fd.display_name} was added.`, time: _now(), msg_type: 'system' });
            _chatRec = await Server.getChatById(_chatId);
            if (_chatRec) { _msgs = _chatRec.data.messages || []; _msgHash = JSON.stringify(_msgs); _renderMessages(); }
